...
class CreditCardResourceIT {
    ...
    void getAllCreditCardsWithEagerRelationshipsIsEnabled() throws Exception {
        when(creditCardServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restCreditCardMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(creditCardServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllCreditCardsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(creditCardServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restCreditCardMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(creditCardRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
